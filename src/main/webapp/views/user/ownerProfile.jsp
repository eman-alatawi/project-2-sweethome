<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<jsp:include page="../shared/_layout.jsp" />

<!-- content -->


<!-- OWNED HOUSES -->



<div class="container-xl flex  flex-row justify-evenly   p-10 ">
	<div class="container-sm flex flex-col justify-center  w-100 h-100 font-mono capitalize text-lg text-gray-700">

		<div>
			<img
				class="object-cover m-10 w-60 h-60 rounded-full mr-10 ring-8  ring-pink-500  ring-opacity-50 "
				src="${owner.userImage}" />
		</div>
		<div class=" pl-2 w-100 border-l-8 border-pink-500">
			<span class="text-sm mr-2">Role:</span>${owner.userRole}</div>
		<div class=" pl-2 w-100 border-l-8 border-yellow-600">
			<span class="text-sm mr-2">First Name:</span>${owner.firstName}</div>
		<div class=" pl-2 w-100 border-l-8 border-indigo-500">
			<span class="text-sm mr-2">Last Name:</span>${owner.lastName}</div>
		<div class=" pl-2 w-100 border-l-8 border-red-400">
			<span class="text-sm mr-2">Email Address:</span>${owner.emailAddress}</div>
		<div class=" pl-2 w-100 border-l-8 border-red-800">
			<span class="text-sm mr-2">Phone Number:</span>${owner.phoneNumber}</div>
			
	   <% if (session.getAttribute("user") != null) {%>

		<%if (session.getAttribute("userRole").equals("owner")) {%>
		<c:if test="${owner.id == user.id}">

			<div class="flex h-10  flex-row justify-evenly w-50 bg-pink-200 pt-2 leading-2">
			<div>
			
				<a href="${appName}user/ownerProfileEdit?id=${owner.id}"><img
					class=" mr-2 transform hover:scale-110 motion-reduce:transform-none"
					width="20" height="20"
					src="data:image/svg+xml;base64," /></a><br></div>
			<span class="text-sm mr-2">Profile</span>
			<div>
			<a href="${appName}user/changePassword?id=${owner.id}"><img class=" mr-2 transform hover:scale-110 motion-reduce:transform-none"
					width="25" height="25" src="data:image/svg+xml;base64," /></a></div>
			<span class="text-sm mr-2 ">Password</span>
			</div>
		</c:if>
		<%}%>
		<%}%>
	</div>


	<div
		class="container-sm flex flex-col justify-center border-l-2 border-fuchsia-900  h-100 w-100 ">
		 <% if( session.getAttribute("userRole").equals("owner")){ %>
		<div class="w-full flex flex-row justify-center mb-2">
      <div class="btn  bg-green-400 font-bold tracking-widest w-48  shadow-sm  text-gray-50 hover:text-gray-100 "><a href="${appName}house/add">Add New House</a></div>
</div>
<% } %>
		<h2
			class="w-100  text-center text-gray-700 mt-3 tracking-widest antialiased">OWNED
			HOUSES</h2>
		<c:forEach items="${owner.getOwnedHouses()}" var="house">
			<div class="container-xl mr-2 ">
				<div class="card mb-3 mt-3 shadow" style="max-width: 600px;">
					<div class="row g-0">
						<div class="col-md-4">
							<img class="w-full h-full object-cover shadow-sm " src="${house.houseImage}"
								alt="House Image">
						</div>
						<div class="col-md-8">
							<div class="card-body">

								<a href="${appName}house/detail?id=${house.id}"
									class="btn  shadow-sm bg-indigo-400 hover:bg-indigo-100 text-gray-50 hover:text-gray-700">More
									Details</a>
									 <div class="px-1 pt-3 pb-4 ">
							              <c:choose>
							                <c:when test="${house.getRentee() != null}">
							                 <div class="text-xs uppercase font-bold text-gray-600 tracking-wide">Rentee</div>
							
							                <div class="flex items-center pt-2">
							                    <a href="${appName}user/renteeProfile?id=${house.rentee.id}" class="">
							                    <div class="bg-cover bg-center w-10 h-10 rounded-full mr-3 ring-4 ring-green-500 ring-opacity-50" style="background-image: url(${house.rentee.userImage})"></div></a>
							
							                    <div>
							                        <p class="font-bold text-gray-900">${house.rentee.firstName} ${house.rentee.lastName}</p>
							                    </div>
							               </div>
							               </c:when>
							               <c:otherwise>
												  <p class="statusFont tracking-widest text-green-500"> No Renters Yet </p>
												</c:otherwise>
							               </c:choose>
                                     </div>
								<div class="flex flex-row w-100 justify-between ">
									<p class="card-text">
										<small class="text-muted ">Last updated:
											${house.updateAt}</small>
									</p>
									<%
									if (session.getAttribute("user") != null) {
									%>
									<%
									if (session.getAttribute("userRole").equals("owner")) {
									%>
									<c:if test="${owner.id == user.id}">
										<div class="flex flex-row w-10 justify-evenly ">
											<div>
												<a href="${appName}house/edit?id=${house.id}"><span class="material-icons mr-5 cursor-pointer  transform hover:scale-110 motion-reduce:transform-none group-hover:text-black" >create</span></a>
											</div>
											<div class="pr-3">
												<a href="${appName}house/delete?id=${house.id}"><span class="material-icons  cursor-pointer  transform hover:scale-110 motion-reduce:transform-none group-hover:text-black" >clear</span></a>
											</div>
										</div>
									</c:if>
									<%
									}
									%>
									<%
									}
									%>
								</div>
							</div>

						</div>
					</div>
				</div>
			</div>

		</c:forEach>
	</div>
</div>
<jsp:include page="../shared/footer.jsp" />
